...
class GsonTest {
    ...
    @Test
    void testLocalTimeFormatting12Hours() {

        Gson gson = GsonConfiguration.getConfiguration(true);
        LocalTime time = LocalTime.of(13, 4);
        String timeString = gson.toJson(time);

        assertThat(timeString).isEqualTo("\"1:04PM\"");

    }

    @Test
    void testLocalTimeFormatting24Hours() {

        Gson gson = GsonConfiguration.getConfiguration(false);
        LocalTime time = LocalTime.of(13, 4);
        String timeString = gson.toJson(time);
